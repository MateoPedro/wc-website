...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    const { data, error } = await sb.from('destinations').select('*').order('order')
    if (error) return res.status(500).json({ error: error.message })
    return res.json(data)
  }

  if (req.method === 'POST') {
    const { data, error } = await sb.from('destinations').insert(req.body).select().single()
    if (error) return res.status(500).json({ error: error.message })
  ...
  if (req.method === 'PUT') {
    ...
    const { data, error } = await sb.from('destinations').update(req.body).eq('id', id).select().single()
    if (error) return res.status(500).json({ error: error.message })
    return res.json(data)
  }
  ...
  if (req.method === 'DELETE') {
    if (!id) return res.status(400).json({ error: 'Missing id' })
    // Remove all photos from storage first
    const { data: photos } = await sb.from('photos').select('storage_path').eq('destination_id', id)
    if (photos?.length) {
      await sb.storage.from('photos').remove(photos.map((p) => p.storage_path))
    }
    await sb.from('photos').delete().eq('destination_id', id)
