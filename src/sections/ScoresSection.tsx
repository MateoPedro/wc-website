...
function AnimatedScore({ score }: { score: number | null }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      step++
      setDisplayed(Math.round((step / steps) * score))
...
function statusBadge(status: FDMatch['status']) {
  if (status === 'IN_PLAY' || status === 'PAUSED') return { label: '● Live', color: '#ff4444', bg: 'rgba(255,68,68,0.12)' }
  if (status === 'FINISHED') return { label: 'Full Time', color: '#00cc44', bg: 'rgba(0,204,68,0.1)' }
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="text-center px-3 shrink-0">
            {(finished || live) && porScore !== null && oppScore !== null ? (
              <div style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 38, letterSpacing: '0.08em', color: '#fff', lineHeight: 1 }}>
                {finished ? <AnimatedScore score={porScore} /> : porScore}
                {' — '}
                {finished ? <AnimatedScore score={oppScore} /> : oppScore}
              </div>
            ) : (
              <div className="text-[11px] tracking-[0.25em] uppercase" style={{ color: 'rgba(255,255,255,0.3)' }}>
                vs
              </div>
            )}
          </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2.5 flex-1">
        ...
        <span className="text-sm" style={{ color: mine ? '#fff' : 'rgba(255,255,255,0.6)', fontWeight: mine ? 600 : 400 }}>
          {row.team.name}
        </span>
      </div>
      {[row.playedGames, row.won, row.draw, row.lost, row.goalDifference].map((v, i) => (
        <span key={i} className="w-8 text-center text-xs" style={{ color: 'rgba(255,255,255,0.35)' }}>
          {typeof v === 'number' && i === 4 ? (v > 0 ? `+${v}` : v) : v}
        </span>
      ))}
      <span className="w-10 text-right text-sm font-semibold tabular-nums" style={{ color: mine ? '#C8A200' : 'rgba(255,255,255,0.6)' }}>
        {row.points}
      </span>
    </motion.div>
  )
...
export default function ScoresSection() {
  ...
  return (
    <div className="relative min-h-screen" style={{ background: '#080808' }}>
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse 60% 50% at 100% 30%, rgba(204,0,0,0.06) 0%, transparent 60%)' }} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-16">
          ...
          <div className="lg:col-span-2">
            <motion.h2
              ...
              style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 'clamp(60px, 8vw, 120px)', lineHeight: 0.9, letterSpacing: '0.02em' }}
            >
              <span className="text-white">The</span>
              <br />
              <span style={{ background: 'linear-gradient(110deg, #CC0000, #ff6666)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                Scores
              </span>
            </motion.h2>
            ...
            <motion.p
              ...
              transition={{ delay: 0.2 }} className="mt-6 text-sm leading-relaxed max-w-xs"
              style={{ color: 'rgba(255,255,255,0.3)', fontWeight: 300 }}
            >
              Portugal Group K results, live scores, and standings. Updated daily.
            </motion.p>
            ...
            {data && data.standings.length > 0 && (
              ...
              >
                <div className="flex items-center px-4 py-2.5" style={{ background: 'rgba(255,255,255,0.03)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                  <span className="w-6" />
                ...
                {data.standings.map((row, i) => <StandingRow key={row.team.id} row={row} index={i} />)}
              </motion.div>
            )}

            {loading && <div className="mt-10 text-xs tracking-widest uppercase animate-pulse" style={{ color: 'rgba(255,255,255,0.2)' }}>Loading…</div>}
            {error && <div className="mt-10 text-xs" style={{ color: 'rgba(204,0,0,0.6)' }}>{error}</div>}
          </div>
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            {data?.matches.map((match, i) => <MatchCard key={match.id} match={match} index={i} />)}
            {data && data.matches.length === 0 && (
              <div className="flex items-center justify-center h-40 rounded-2xl" style={{ border: '1px solid rgba(255,255,255,0.07)' }}>
                <p className="text-sm" style={{ color: 'rgba(255,255,255,0.3)' }}>No fixtures found yet</p>
              </div>
            )}
          </div>
        </div>
